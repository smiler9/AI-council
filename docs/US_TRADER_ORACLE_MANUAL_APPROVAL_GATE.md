...
- [ ] connectivity strategy가 outbox-only 또는 Mac pull 중심인지 확인
...
## Connectivity strategy 확인

```bash
scripts/run_oracle_connectivity_strategy_dryrun.sh
```

이 dry-run은 실제 tunnel, public endpoint, SSH reverse tunnel을 만들지 않고 전략 비교와 plan 검증만 수행합니다.
...
- `order_execution_allowed` 값이 `true`가 될 가능성 발견
