...
1. Oracle에서 AI Council endpoint 접근성만 read-only로 확인
2. preview sidecar directory 생성 전 최종 manual approval record 작성
3. maintenance window에서 preview-only sidecar 파일 수동 복사
4. sidecar run-once `--mode preview`만 실행
5. AI Council normalize-preview 응답과 adapter warning 확인
