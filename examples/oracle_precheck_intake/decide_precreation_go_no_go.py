...
DEFAULT_OUTPUT = ROOT / "tmp" / "oracle_precheck_intake" / "go_no_go_decision.json"
SAFETY_BOUNDARY = (
    "AI Council은 거래를 실행하거나 브로커 API에 연결하지 않습니다. "
    "이 결과는 검토, 리스크 분석, 의사결정 보조 목적으로만 사용됩니다."
)
NEXT_PHASE = "Phase 24P Oracle outbox manual precreation command packet"


def main() -> int:
    parser = argparse.ArgumentParser(description="Decide GO/NO_GO for Oracle outbox precreation manual review.")
    ...
    try:
        decision = decide(Path(args.intake))
...
def decide(intake_path: Path) -> dict[str, Any]:
    intake = json.loads(intake_path.expanduser().read_text(encoding="utf-8"))
    validation = run_validation(intake_path)
    ...
    if intake.get("result_status") not in {"passed", "warning"}:
        reasons.append(f"result_status={intake.get('result_status')}")
    if intake.get("result_status") == "warning":
        warnings.append("result_status=warning requires extra manual acknowledgement before any later write step")
    ...
    return {
        ...
        "decision_scope": "Allows only the next manual review stage, not deployment or order execution.",
        "reasons": reasons,
        "warnings": warnings,
        "validation": validation,
        "required_manual_acknowledgements": [
            "GO is not deployment approval.",
            "GO does not permit broker API connection or order execution.",
            "GO does not permit live bot modification or systemd changes.",
            "Any future Oracle write step requires a separate manual approval.",
        ],
...
def run_validation(intake_path: Path) -> dict[str, Any]:
    script = INTAKE_DIR / "validate_precheck_intake.py"
    result = subprocess.run(
        [sys.executable, str(script), "--intake", str(intake_path.expanduser().resolve())],
        check=False,
        capture_output=True,
        text=True,
    )
