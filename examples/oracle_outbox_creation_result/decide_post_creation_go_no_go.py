...
DEFAULT_OUTPUT = ROOT / "tmp" / "oracle_outbox_creation_result" / "post_creation_go_no_go_decision.json"
NEXT_PHASE = "Phase 24R Oracle preview signal file write rehearsal"
SAFETY_BOUNDARY = (
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Decide GO/NO_GO after Oracle outbox creation verification.")
...
def decide(result_path: Path) -> dict[str, Any]:
    ...
    if payload.get("result_status") == "warning":
        warnings.append("result_status=warning requires extra manual acknowledgement before any later write rehearsal")
    for key in [
        "outbox_dir_exists",
    ...
    return {
        ...
        "decision_scope": "Allows only preview signal file write rehearsal, not live bot patching or order execution.",
        "reasons": reasons,
        "warnings": warnings,
        ...
        "required_manual_acknowledgements": [
            ...
            "The next phase is preview signal file write rehearsal only.",
        ],
...
def run_validation(result_path: Path) -> dict[str, Any]:
    script = RESULT_DIR / "verify_creation_result.py"
